<script lang="ts">
  ...
  async function loadPosts(reset = true) {
    console.log(`🔄 [Explore/loadPosts] reset=${reset}, currentPage=${currentPage}`);
    if (reset) {
      loading = true;
      currentPage = 1;
      posts = [];
    ...
    try {
      console.log(`📡 [Explore/API Call] page=${currentPage}, limit=20`);
      ...
      console.log(`✅ [Explore/API Response] received ${newPosts.length} posts`);
      ...
      console.log(`📊 [Explore/State] Total posts: ${posts.length}, hasMore: ${hasMore}`);
    } catch (err) {
      error = err instanceof Error ? err.message : "Failed to load posts";
      console.error("❌ [Explore/Error] loading posts:", err);
    } finally {
      loading = false;
      loadingMore = false;
    }
  ...
  async function loadMorePosts() {
    console.log(`🔽 [Explore/loadMorePosts] called - loadingMore=${loadingMore}, hasMore=${hasMore}`);
    if (loadingMore || !hasMore) {
      console.log(`⛔ [Explore/loadMorePosts] blocked`);
      return;
    }
    currentPage++;
    console.log(`➕ [Explore/loadMorePosts] incrementing page to ${currentPage}`);
  ...
  $effect(() => {
    if (!sentinelElement || loading) {
      console.log(`⏸️ [Explore/Observer] not initialized - sentinelElement=${!!sentinelElement}, loading=${loading}`);
      return;
    }

    console.log(`👁️ [Explore/Observer] initializing...`);

    const observer = new IntersectionObserver(
      (entries) => {
        console.log(`🎯 [Explore/Observer] callback triggered - isIntersecting=${entries[0].isIntersecting}, hasMore=${hasMore}, loadingMore=${loadingMore}`);
        if (entries[0].isIntersecting && hasMore && !loadingMore) {
          console.log(`✅ [Explore/Observer] calling loadMorePosts()`);
          loadMorePosts();
        }
      },
      { threshold: 0.1 },
    ...
    console.log(`✅ [Explore/Observer] observing sentinel element`);

    return () => {
      console.log(`🛑 [Explore/Observer] disconnecting`);
      observer.disconnect();
    };
  });
